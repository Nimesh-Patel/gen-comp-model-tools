#' Move thresholded popsize between named states
#'
#' Move populations between some number of named updatedstates and to another
#' single named state. Amount moved is up to specifed popsizeval ensuring no
...
movethresholdpopsize_byname <- function(tblpopsize,
                                        popsizeval,
                                        namebefore,
                                        nameafter) {
  ...
  popsizeval <- sapply(popbefore, function(x) {
    min(x, popsizeval)
  })
  ...
  tblpopsize[["popsize"]][logicafter] <-
    tblpopsize[["popsize"]][logicafter] + sum(popsizeval)
