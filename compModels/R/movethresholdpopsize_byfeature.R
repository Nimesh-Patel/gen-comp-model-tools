#' Move thresholded popsize for between states matching input features
#'
...
#' Checks ensure proper input argument combinations. Amount moved is up to
#' specifed movepopsize ensuring no negative populations
#'
...
movethresholdpopsize_byfeature <- function(tblpopsize, # nolint: cyclocomp_linter, line_length_linter.
  ...
  for (rowidx in seq_len(nrow(tbljoin))) {
    ...
    currmovepopsize <- min(movepopsize, currpop)
    tblpopsize[["popsize"]][beforelogic] <-
      tblpopsize[["popsize"]][beforelogic] - currmovepopsize
    tblpopsize[["popsize"]][afterlogic] <-
      tblpopsize[["popsize"]][afterlogic] + currmovepopsize
  }
